  ...
  return async function handleLead(payload) {
    ...
    try {
      ...
      var dealId = await addCrmRecord(fetchImpl, baseUrl, "crm.deal.add", {
        ...
        SOURCE_ID: prepared.deal.SOURCE_ID,
        SOURCE_DESCRIPTION: prepared.deal.SOURCE_DESCRIPTION,
        COMMENTS: prepared.deal.COMMENTS,
        CONTACT_IDS: [contactId]
      });
      try {
        await addCrmRecord(fetchImpl, baseUrl, "crm.timeline.comment.add", {
          ENTITY_ID: dealId,
          ENTITY_TYPE: "deal",
          COMMENT: prepared.deal.COMMENTS
        });
      } catch (_) {
        // The deal already exists and carries the same data in COMMENTS.
      }
